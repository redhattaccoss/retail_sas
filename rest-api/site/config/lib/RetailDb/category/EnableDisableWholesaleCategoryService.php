<?php
class EnableDisableWholesaleCategoryService extends Service{
	...
	protected function runImplementation(){
		//enable - the value to be used in setting a category in wholesale
		...
		if ((trim($enable)!="")&&(trim($name)!="")){
			$category = CategoryIQL::select()->where(CategoryIQL::$_TABLE, CategoryIQL::$NAME, "=", trim($name))->_and(CategoryIQL::$_TABLE, CategoryIQL::$WHOLESALEENABLED, "=", "1")->getFirst();
